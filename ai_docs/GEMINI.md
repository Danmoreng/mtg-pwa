...
- Use Conventional Commits with `[Gemini]` prefix, e.g., `[Gemini] feat(import): add idempotency key`.
...
- **DO**: Use **Bootstrap 5 utility classes** for styling.
...
Additionally, for each proposed commit message, prefix with `[Gemini]` and use Conventional Commits, e.g.:
```
[Gemini] feat(db): introduce Dexie schema v1 and money helpers
```
...
- **Styling**: Use **Bootstrap 5 utility classes** for layout and styling. For components use Bootstrap components when appropriate.
...
2. **Use Bootstrap components** for simple UI elements like buttons, cards, and navigation
3. **Avoid custom CSS** unless absolutely necessary - prefer utility classes
