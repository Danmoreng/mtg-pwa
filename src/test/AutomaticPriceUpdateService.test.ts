...
import { AutomaticPriceUpdateService } from '../src/features/pricing/AutomaticPriceUpdateService';
import { settingRepository } from '../src/data/repos';
import { PriceUpdateService } from '../src/features/pricing/PriceUpdateService';
...
vi.mock('../src/features/pricing/PriceUpdateService', () => ({
  PriceUpdateService: {
    syncPrices: vi.fn()
  }
}));
...
describe('AutomaticPriceUpdateService', () => {
  ...
  describe('needsPriceUpdate', () => {
    it('should return true when no last update timestamp exists', async () => {
      (settingRepository.get as jest.Mock).mockResolvedValue(null);
    ...
    it('should return true when more than 24 hours have passed since last update', async () => {
      ...
      (settingRepository.get as jest.Mock).mockResolvedValue(twentyFiveHoursAgo);
    ...
    it('should return false when less than 24 hours have passed since last update', async () => {
      ...
      (settingRepository.get as jest.Mock).mockResolvedValue(twelveHoursAgo);
    ...
    it('should return true when there is an error checking the last update time', async () => {
      (settingRepository.get as jest.Mock).mockRejectedValue(new Error('Database error'));
  ...
  describe('updatePrices', () => {
    it('should update prices and record the update time', async () => {
      (PriceUpdateService.syncPrices as jest.Mock).mockResolvedValue(undefined);
      (settingRepository.set as jest.Mock).mockResolvedValue(undefined);
    ...
    it('should throw an error when price update fails', async () => {
      (PriceUpdateService.syncPrices as jest.Mock).mockRejectedValue(new Error('Network error'));
  ...
  describe('getLastUpdateTime', () => {
    it('should return the last update time when it exists', async () => {
      ...
      (settingRepository.get as jest.Mock).mockResolvedValue(timestamp);
  ...
  describe('getNextUpdateTime', () => {
    ...
    it('should return 24 hours after the last update time', async () => {
      ...
      (settingRepository.get as jest.Mock).mockResolvedValue(lastUpdate.toISOString());
