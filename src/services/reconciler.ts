...
export async function findOrCreateProvisionalLot(
  ...
  // Create a new provisional lot
  const newLot: Omit<CardLot, 'id'> = {
    cardId: identity.cardId || '',
    acquisitionId,
    quantity: 1, // Default quantity for provisional lots
    condition: 'NM', // Default condition
    language: identity.lang,
    foil: identity.finish === 'foil', // Set foil based on finish
    finish: identity.finish as 'nonfoil' | 'foil' | 'etched',
    unitCost: 0, // Provisional lots have zero cost initially
    source: 'provisional',
    purchasedAt: when,
    createdAt: new Date(),
    updatedAt: new Date()
  ...
  const lotId = await cardLotRepository.add(newLot as CardLot);
  return await cardLotRepository.getById(lotId) as CardLot;
}
