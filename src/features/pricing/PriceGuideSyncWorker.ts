// PriceGuideSyncWorker handles daily Cardmarket Price Guide ingestion
import db from '../../data/db';
import { cardRepository } from '../../data/repos';
import { ValuationEngine } from '../analytics/ValuationEngine';

export class PriceGuideSyncWorker {
  private static readonly BATCH_SIZE = 50;
  
  // Sync daily Cardmarket Price Guide data
  static async syncPriceGuide(progressCallback?: (processed: number, total: number) => void): Promise<void> {
    try {
      ...
      for (let i = 0; i < cards.length; i += this.BATCH_SIZE) {
        ...
        for (const card of batch) {
          ...
          try {
            // Get Price Guide data for this card
            await this.processCardPriceGuideData(card);
          } catch (error) {
      ...
      // Create a valuation snapshot after successful Price Guide sync
      try {
        await ValuationEngine.createValuationSnapshot();
        console.log('Valuation snapshot created after Price Guide sync');
      } catch (error) {
        console.error('Error creating valuation snapshot after Price Guide sync:', error);
      }
    } catch (error) {
  ...
  private static async processCardPriceGuideData(card: any): Promise<void> {
    try {
  ...
  private static createPricePoint(
    cardId: string,
    date: string,
